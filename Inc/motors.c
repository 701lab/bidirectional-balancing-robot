...
float motor_get_speed_by_incements(const motor *motor_instance, const float time_increment)
{
	...
	int16_t current_counter_value = motor_instance->get_encoder_counter_val();
	int16_t encoder_counter_increment =  current_counter_value - motor_instance->speed_controller->previous_encoder_counter_value;
	...
	motor_instance->speed_controller->previous_encoder_counter_value = current_counter_value;
...
float motors_speed_controller_handler(const motor *motor_instance, const float time_increment)
{
	...
	float speed_limitation = motor_instance->speed_controller->controller_output_limitat;
...
uint32_t motors_rotation_deiraction_test (motor *motor_instance)
{
	...
	int16_t current_encoder_counter_value = motor_instance->get_encoder_counter_val();

	// Start motor with 1/2 of power
	motor_instance->set_pwm_duty_cycle(motor_instance->max_duty_cycle/2);
	...
	int16_t new_current_encoder_counter_value = motor_instance->get_encoder_counter_val();
